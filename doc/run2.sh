...
# Ex: all | admin | batch | worker | sse | api
appName=$2
...
function cleanLog(){
    echo " "
    echo -e "\033[35m start clean log \033[0m"
    if [[ "admin" == "$appName" ]]; then
        rm -rf $logPath/server-console/*crm-web-admin*
        rm -rf $logPath/server-start/*crm-web-admin*
        rm -rf $logPath/server-error/*crm-web-admin*
        rm -rf $logPath/server-warn/*crm-web-admin*
    elif [[ "batch" == "$appName" ]]; then
        rm -rf $logPath/server-console/*crm-batch*
        rm -rf $logPath/server-start/*crm-batch*
        rm -rf $logPath/server-error/*crm-batch*
        rm -rf $logPath/server-warn/*crm-batch*
    elif [[ "worker" == "$appName" ]]; then
        rm -rf $logPath/server-console/*crm-worker*
        rm -rf $logPath/server-start/*crm-worker*
        rm -rf $logPath/server-error/*crm-worker*
        rm -rf $logPath/server-warn/*crm-worker*
    elif [[ "api" == "$appName" ]]; then
		rm -rf $logPath/server-console/*crm-web-api*
        rm -rf $logPath/server-start/*crm-web-api*
        rm -rf $logPath/server-error/*crm-web-api*
        rm -rf $logPath/server-warn/*crm-web-api*
    else
        rm -rf $logPath/*
        mkdir $logPath/server-start
    fi
    echo -e "\033[35m end clean log \033[0m"
    echo " "
}

function cleanDB(){
    echo " "
    echo -e "\033[35m start clean database \033[0m"
    mysqlParams="-u crm -h localhost -padmin"
    if [[ "production" == "$serverProfiles" ]]; then
        mysqlParams="-u crm -h crm-db.cp4y2mv6moha.rds.cn-north-1.amazonaws.com.cn -pfanfever"
    elif [[ "staging" == "$serverProfiles" ]]; then
        mysqlParams="-u crm -h localhost -ppassword"
    elif [[ "frontend" == "$serverProfiles" ]]; then
        mysqlParams="-u root -h localhost -proot"    
    else
        echo ""
    fi
...
EOF
    echo -e "\033[35m end clean database \033[0m"
    echo " "
    echo -e "\033[35m start clean elasticsearch data \033[0m"
    curl -XDELETE 'http://localhost:9200/leads/leads/_query' -d '{"query":{"match_all":{}}}'
    curl -XDELETE 'http://localhost:9200/cust/customer/_query' -d '{"query":{"match_all":{}}}'
    curl -XDELETE 'http://localhost:9200/niche/niche/_query' -d '{"query":{"match_all":{}}}'
    curl -XDELETE 'http://localhost:9200/contacts/contacts/_query' -d '{"query":{"match_all":{}}}'
    echo -e "\033[35m end clean elasticsearch data \033[0m"
    echo " "
}
...
#            rm -rf $runPidPath/crm-web-admin-staging2.pid
            ...
            echo -e "\033[35m Stoping process $(cat $runPidPath/crm-web-api-staging.pid) \033[0m"
            kill $(cat $runPidPath/crm-web-api-staging.pid)

            sleep 5

            kill -9 $(cat $runPidPath/crm-web-api-staging.pid)
            rm -rf $runPidPath/crm-web-api-staging.pid
...
function copyFiles(){
    ...
    if [[ "admin" == "$appName" ]]; then
	    ...
	    cp $sourceCodePath/crm/crm-web-api/target/crm-web-api-0.0.1-SNAPSHOT.jar $runPath/crm-web-api-0.0.1-SNAPSHOT.jar
    else
        if [[ "staging" == "$serverProfiles" || "production" == "$serverProfiles" ]]; then
            cp $sourceCodePath/crm/crm-web-admin/target/crm-web-admin-0.0.1-SNAPSHOT.jar $runPath/crm-web-admin-0.0.1-SNAPSHOT-1.jar
            cp $sourceCodePath/crm/crm-web-admin/target/crm-web-admin-0.0.1-SNAPSHOT.jar $runPath/crm-web-admin-0.0.1-SNAPSHOT-2.jar
...
function run(){
    ...
    if [[ "production" == "$serverProfiles" ]]; then
        if [[ "admin" == "$appName" ]]; then
            nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-1.jar --spring.profiles.active=production1 > $logPath/server-start/crm-web-admin-production1-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-production1.pid
            ...
            \033[0m"
            nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-2.jar --spring.profiles.active=production2 > $logPath/server-start/crm-web-admin-production2-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-production2.pid
            ...
            nohup nice java -jar crm-batch-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-batch-production-start.log 2>&1 & echo $! > $runPidPath/crm-batch-production.pid
            ...
            nohup nice java -jar crm-worker-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-worker-production-start.log 2>&1 & echo $! > $runPidPath/crm-worker-production.pid
            ...
            nohup nice java -jar crm-web-sse-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-web-sse-production-start.log 2>&1 & echo $! > $runPidPath/crm-web-sse-production.pid
            ...
            nohup nice java -jar crm-web-api-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-web-api-production-start.log 2>&1 & echo $! > $runPidPath/crm-web-api-production.pid
            echo -e "\033[35m start server nohup nice java -jar crm-web-api-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-web-api-production-start.log 2>&1 & echo $! > $runPidPath/crm-web-api-production.pid
            \033[0m"
	    else
            nohup nice java -jar crm-web-sse-0.0.1-SNAPSHOT.jar --spring.profiles.active=production1 > $logPath/server-start/crm-web-sse-production-start.log 2>&1 & echo $! > $runPidPath/crm-web-sse-production.pid
            echo -e "\033[35m start server nohup nice java -jar crm-web-sse-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-web-sse-production-start.log 2>&1 & echo $! > $runPidPath/crm-web-sse-production.pid
            \033[0m"
            nohup nice java -jar crm-batch-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-batch-production-start.log 2>&1 & echo $! > $runPidPath/crm-batch-production.pid
            echo -e "\033[35m start server nohup nice java -jar crm-batch-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-batch-production-start.log 2>&1 & echo $! > $runPidPath/crm-batch-production.pid
            ...
            echo -e "\033[35m start server nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-2.jar --spring.profiles.active=production2 > $logPath/server-start/crm-web-admin-production2-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-production2.pid
            \033[0m"
			nohup nice java -jar crm-web-api-0.0.1-SNAPSHOT.jar --spring.profiles.active=production1 > $logPath/server-start/crm-web-api-production-start.log 2>&1 & echo $! > $runPidPath/crm-web-api-production.pid
			echo -e "\033[35m start server nohup nice java -jar crm-web-api-0.0.1-SNAPSHOT.jar --spring.profiles.active=production > $logPath/server-start/crm-web-api-production-start.log 2>&1 & echo $! > $runPidPath/crm-web-api-production.pid
			\033[0m"
        fi 
    elif [[ "staging" == "$serverProfiles" ]]; then
        if [[ "admin" == "$appName" ]]; then
            nohup nice java -Xmx256m -jar crm-web-admin-0.0.1-SNAPSHOT-1.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-admin-staging1-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-staging1.pid
            echo -e "\033[35m start server nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-1.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-admin-staging1-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-staging1.pid
            \033[0m"
#            nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-2.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-admin-staging2-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-staging2.pid
#            echo -e "\033[35m start server nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-2.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-admin-staging2-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-staging2.pid
#            \033[0m"
        elif [[ "batch" == "$appName" ]]; then
            nohup nice java -Xmx256m -jar crm-batch-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-batch-staging-start.log 2>&1 & echo $! > $runPidPath/crm-batch-staging.pid
            echo -e "\033[35m start server nohup nice java -jar crm-batch-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-batch-staging-start.log 2>&1 & echo $! > $runPidPath/crm-batch-staging.pid
            \033[0m"
        elif [[ "worker" == "$appName" ]]; then
            nohup nice java -Xmx256m -jar crm-worker-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-worker-staging-start.log 2>&1 & echo $! > $runPidPath/crm-worker-staging.pid
            echo -e "\033[35m start server nohup nice java -jar crm-worker-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-worker-staging-start.log 2>&1 & echo $! > $runPidPath/crm-worker-staging.pid
            \033[0m"
        elif [[ "sse" == "$appName" ]]; then
            nohup nice java -Xmx256m -jar crm-web-sse-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-sse-staging-start.log 2>&1 & echo $! > $runPidPath/crm-web-sse-staging.pid
            echo -e "\033[35m start server nohup nice java -jar crm-web-sse-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-sse-staging-start.log 2>&1 & echo $! > $runPidPath/crm-web-sse-staging.pid
            \033[0m"
        elif [[ "api" == "$appName" ]]; then
            nohup nice java -Xmx256m -jar crm-web-api-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-api-staging-start.log 2>&1 & echo $! > $runPidPath/crm-web-api-staging.pid
	        echo -e "\033[35m start server nohup nice java -jar crm-web-api-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-api-staging-start.log 2>&1 & echo $! > $runPidPath/crm-web-api-staging.pid
	        \033[0m"
        else
            nohup nice java -Xmx256m -jar crm-web-sse-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-sse-staging-start.log 2>&1 & echo $! > $runPidPath/crm-web-sse-staging.pid
            echo -e "\033[35m start server nohup nice java -jar crm-web-sse-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-sse-staging-start.log 2>&1 & echo $! > $runPidPath/crm-web-sse-staging.pid
            ...
            \033[0m"
            nohup nice java -Xmx256m -jar crm-worker-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-worker-staging-start.log 2>&1 & echo $! > $runPidPath/crm-worker-staging.pid
            echo -e "\033[35m start server nohup nice java -jar crm-worker-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging > $logPath/server-start/crm-worker-staging-start.log 2>&1 & echo $! > $runPidPath/crm-worker-staging.pid
            \033[0m"
            nohup nice java -Xmx256m -jar crm-web-admin-0.0.1-SNAPSHOT-1.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-admin-staging1-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-staging1.pid
            echo -e "\033[35m start server nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-1.jar --spring.profiles.active=staging1 > $logPath/server-start/crm-web-admin-staging1-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-staging1.pid
            \033[0m"
            nohup nice java -Xmx256m -jar crm-web-api-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-api-staging-start.log 2>&1 & echo $! > $runPidPath/crm-web-api-staging.pid
            echo -e "\033[35m start server nohup nice java -jar crm-web-api-0.0.1-SNAPSHOT.jar --spring.profiles.active=staging > $logPath/server-start/crm-web-api-staging-start.log 2>&1 & echo $! > $runPidPath/crm-web-api-staging.pid
            \033[0m"
#            nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-2.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-admin-staging2-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-staging2.pid
#            echo -e "\033[35m start server nohup nice java -jar crm-web-admin-0.0.1-SNAPSHOT-2.jar --spring.profiles.active=staging2 > $logPath/server-start/crm-web-admin-staging2-start.log 2>&1 & echo $! > $runPidPath/crm-web-admin-staging2.pid
#            \033[0m"
