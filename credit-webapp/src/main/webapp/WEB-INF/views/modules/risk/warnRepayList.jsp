...
<ul id="nav-tabs" class="nav nav-tabs">
     ...
     <li class="active"><a onclick="showInfo('${ctx}warn/repaylist');" data-toggle="tab">逾期预警</a></li>
...
<div id="tab-content" class="tab-content">
    <div id="home" class="tab-pane fade active in">
        ...
        <table class="table table-bordered table-striped">
                ...
                <th width="40" class="text-center bg-primary dropdown"><a href="javascript:;" id="custom-dropdown" class="ico ico-menu"></a>
                    <ul id="custom-menu" class="dropdown-menu">
                        ...
                        <li class="checkbox"><a href="javascript:;">
                            ...
                            <label>
                                <input type="checkbox" checked>当期应还金额（元）
                            </label></a></li>
                        <li class="divider"></li>
            ...
            <c:forEach items="${result.paging.list}" var="warn" varStatus="status">
                <tr>
                    ...
                    <td><a href="javascript:;" class="add-tab">${warn.loanNo}</a></td>
                    <td>${warn.overDays}</td>
                    <td>${warn.custName}</td>
                    <td>${warn.custTel}</td>
                    <td>${warn.prodName}</td>
                    ...
                    <td><fmt:formatDate value="${warn.repaymentTime}" type="date"/></td>
                    <td>${warn.expectAmt}</td>
                    <td>
        ...
        <nav>
            <ul class="pagination">
                <li><a href="${ctx}warn/repaylist?pageNum=1">首页</a></li>
                <c:if test="${result.paging.pageNum > 1}">
                    <li><a href="${ctx}warn/repaylist?pageNum=${result.paging.pageNum - 1}" aria-label="Previous">&laquo;</a></li>
                    <li><a href="${ctx}warn/repaylist?pageNum=${result.paging.pageNum - 1}">${result.paging.pageNum - 1}</a></li>
                </c:if>
                <li class="active"><a href="javascript:;">${result.paging.pageNum}</a></li>

                <c:if test="${result.paging.pageNum + 1 <= result.paging.pageCount}">
                    <li><a href="${ctx}warn/repaylist?pageNum=${result.paging.pageNum + 1}">${result.paging.pageNum + 1}</a></li>
                    <li><a href="${ctx}warn/repaylist?pageNum=${result.paging.pageNum + 1}" aria-label="Next">&raquo;</a></li>
                </c:if>
                <li><a href="${ctx}warn/repaylist">最后一页</a></li>
